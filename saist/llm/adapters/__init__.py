...
class BaseLlmAdapter:
    ...
    async def prompt_structured(self, prompt: str, response_format: Type[BaseModel], tool_fns: Optional[List[Callable]] = None) -> BaseModel:
        ...
        agent = Agent(self.model, result_type = response_format, tools=tools)
        response = await agent.run(prompt, model_settings=self.get_model_options())
        logger.getChild(self.__class__.__name__).debug("prompt_structured response", extra={'response_data': response.data, 'prompt': prompt})
    ...
    async def prompt(self, prompt: str, tool_fns: Optional[List[Callable]] = None) -> str | None:
        ...
        response = await agent.run(prompt, model_settings=self.get_model_options())
        logger.getChild(self.__class__.__name__).debug("prompt response", extra={'response_data': response.data, 'prompt': prompt})
        return response.data

...
class FilesystemAdapter(BaseScmAdapter):
    ...
    def __init__(
        self,
        compare_path: PathLike[AnyStr] | str,
        base_path: Optional[PathLike[AnyStr] | str] = None,
    ):
    ...
    def _iter_changed_files(self) -> list[File]:
        logger.debug(
            f"Iterate changed files: base:{self.base_path}, compare:{self.compare_path}"
        )
        ...
        if self.base_path is None:
            ...
            for filename in [
                path for path in diffs if os.path.isfile(f"{self.compare_path}/{path}")
            ]:
        ...
        for filename in diffs.diff_files:
            ...
            try:
                with open(a_path, "r") as a, open(b_path, "r") as b:
                    patch = "".join(
                        difflib.unified_diff(
                            a.readlines(), b.readlines(), fromfile="", tofile=""
                        )
                    )

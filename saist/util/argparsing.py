...
filesystem_parser = SCM_subparsers.add_parser(
    "poem", help="Forget code scanning, get a nice little DevSecOps poem"
)

### FILESYTEM
filesystem_parser = SCM_subparsers.add_parser(
    "filesystem", help="Scan a local folder on disk"
)
filesystem_parser.add_argument(
    "path",
    type=str,
    help="Path to the folder to be scanned",
    envvar="SAIST_FILESYSTEM_PATH",
    action=EnvDefault,
)
filesystem_parser.add_argument(
    "--path-for-comparison",
    type=str,
    help="Path to generate diff with",
    envvar="SAIST_COMPARE_PATH",
    action=EnvDefault,
    required=False,
)
...
### GIT
git_parser = SCM_subparsers.add_parser(
    "git", help="Scan a local git repository on disk"
)
...
git_parser.add_argument(
    "--ref-for-compare",
    type=str,
    help="Git ref to compare from",
    envvar="SAIST_GIT_BASE_REF",
    action=EnvDefault,
    default="main",
)
git_parser.add_argument(
    "--ref-to-compare",
    type=str,
    help="Git ref to compare to",
    envvar="SAIST_GIT_COMPARE_REF",
    action=EnvDefault,
    default="HEAD",
)
git_parser.add_argument(
    "--commit-for-compare",
    type=str,
    help="Git commit to compare from (preferred over REF if set)",
    envvar="SAIST_GIT_BASE_COMMIT",
    action=EnvDefault,
)
git_parser.add_argument(
    "--commit-to-compare",
    type=str,
    help="Git commit to compare to (preferred over REF if set)",
    envvar="SAIST_GIT_COMPARE_COMMIT",
    action=EnvDefault,
)
...
github_parser.add_argument(
    "--github-token",
    type=str,
    help="Github API token (can be set with env var GITHUB_TOKEN)",
    envvar="GITHUB_TOKEN",
    action=EnvDefault,
    required=True,
)
...
parser.add_argument(
    "--openai-base-uri",
    type=str,
    help="Base uri of openai to use any compatable service",
    envvar="SAIST_OPENAI_BASE_URI",
    action=EnvDefault,
    required=False,
)
...
parser.add_argument(
    "--interactive",
    help="Spawn an interactive prompt with the LLM at the end",
    required=False,
    action="store_true",
)
...
parser.add_argument(
    "--disable-tools",
    help="Disable usage of tools during code analysis (this is a good cost saving)",
    required=False,
    action="store_true",
)
...
parser.add_argument(
    "--web",
    help="Launch a web server to display findings",
    required=False,
    action="store_true",
)
...
parser.add_argument(
    "--ci",
    help="Exit 1 if findings identified",
    required=False,
    action="store_true",
)
...
parser.add_argument(
    "--csv",
    help="Write results to findings.csv",
    required=False,
    action="store_true",
)
...
parser.add_argument(
    "--tex", help="Write results of TeX file", required=False, action="store_true"
)
...
parser.add_argument(
    "--pdf", help="Write results of PDF report", required=False, action="store_true"
)

parser.add_argument(
    "--pdf-filename",
    type=str,
    help="Filename of PDF report",
    envvar="SAIST_PDF_FILENAME",
    action=EnvDefault,
    required=False,
    default="report.pdf",
)
...
parser.add_argument(
    "--disable-caching",
    help="Disable local caching of results",
    action="store_true",
    required=False,
)
...
parser.add_argument(
    "--cache-folder",
    type=str,
    help="Folder name for local caching",
...
parser.add_argument(
    "--project-name",
    type=str,
    help="Project name for pdf output",
    envvar="SAIST_PROJECT_NAME",
    action=EnvDefault,
    required=False,
    default=None,
)
...
parser.add_argument(
    ...
    help="Skip checking files for a maximum line length",
    action="store_true",
    required=False,
)
...
parser.add_argument(
    ...
    "--include",
    action="append",
    help="Pattern or filename to explicitly include, overrides saist.ignore",
    nargs=1,
    required=False,
)
...
parser.add_argument(
    ...
    "--exclude",
    action="append",
    help="Pattern or filename to explicitly ignore, overrides saist.include",
    nargs=1,
    required=False,
)
...
parser.add_argument(
    "--dry-run",
    help="Exit after parsing configuration and collecting files, does not perform any analysis. Useful for validating rules.",
    action="store_true",
    required=False,
)
...
def parse_args():
    ...
    if args.llm == "bedrock" and args.llm_api_key:
        parser.error(
            f"Do not provide an API key for bedrock, use AWS ENV variables https://docs.aws.amazon.com/cli/v1/userguide/cli-configure-envvars.html"
        )

    if args.llm == "bedrock" and args.interactive:
        parser.error(
            "Sorry, we dont support interactive mode with bedrock as AWS tool calling is a bit broken"
        )

    if args.llm not in ["ollama", "bedrock", "faike"] and args.llm_api_key is None:
        parser.error(
            f"You must provide an api key with --llm-api-key if using {args.llm}"
        )
    ...
    if args.pdf and which("latexmk") == None:
        parser.error(
            "Unable to find 'latexmk' binary in $PATH needed for PDF report building, cannot use --pdf flag"
        )
